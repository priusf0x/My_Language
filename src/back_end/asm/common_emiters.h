...
#define EMIT_SETE_BL() \
    ...
    emit_sete_bl(compiler->code_section);} while(0)

#define EMIT_SETNE_BL() \
    do {NASM_EMIT("\tsetne BL\n");\
    emit_setne_bl(compiler->code_section);} while(0)
                           
#define EMIT_SETG_BL() \
    do {NASM_EMIT("\tsetg BL\n");\
    emit_setg_bl(compiler->code_section);} while(0)
                           
#define EMIT_SETGE_BL() \
    do {NASM_EMIT("\tsetge BL\n");\
    emit_setge_bl(compiler->code_section);} while(0)
...
#define EMIT_SETLE_BL() \
    ...
    emit_setle_bl(compiler->code_section);} while(0)

#define EMIT_SETL_BL() \
    do {NASM_EMIT("\tsetl BL\n");\
    emit_setl_bl(compiler->code_section);} while(0)
...
#define EMIT_MOV_REG_REG(___REG_L___, ___REG_R___) \
    ...
    emit_mov(compiler->code_section, ___REG_L___, ___REG_R___);} while(0)

#define EMIT_MOV_REG_CONST(___REG___, ___CONST___)\
    do {NASM_EMIT("\tmov %s, %ld\n", \
        REG_NAMES[(___REG___)], ___CONST___);    \
    emit_mov(compiler->code_section, (___REG___), (___CONST___)); } while (0)
...
#define EMIT_MOV_MEM_OFF_REG(___REG_B___, ___OFFSET___, ___REG___)\
    ...
    emit_mov_mem(compiler->code_section, (___REG_B___), (___OFFSET___), ___REG___);} while (0);

// -------------------------------- push/pop ----------------------------------

#define EMIT_PUSH_REG(___REG___) \
    do {NASM_EMIT("\tpush %s\n", REG_NAMES[(___REG___)]);\
    emit_push(compiler->code_section, (___REG___));} while(0)

#define EMIT_POP_REG(___REG___) \
    do {NASM_EMIT("\tpop %s\n", REG_NAMES[(___REG___)]);\
    emit_pop(compiler->code_section, (___REG___));} while(0)

// ----------------------------- call/ret -------------------------------------

#define EMIT_CALL(___STRING___)\
do {NASM_EMIT("\tcall %.*s\n", (int) (___STRING___).size, (___STRING___).string);\
    SetPlaceholder(compiler, (___STRING___), compiler->code_section->cur_pos, \
                        &compiler->placehldr.func);\
    emit_call(compiler->code_section, 0x0);} while (0)

#define EMIT_RET()\
    do {NASM_EMIT( "\tret\n");\
    emit_ret(compiler->code_section);} while(0) 
...
#define EMIT_ADD_REG_REG(___REG_L___, ___REG_R___) \
    do {NASM_EMIT("\tadd %s, %s\n", \
        REG_NAMES[(___REG_L___)], REG_NAMES[(___REG_R___)]);\
    emit_add(compiler->code_section, ___REG_L___, ___REG_R___);} while (0)

#define EMIT_SUB_REG_REG(___REG_L___, ___REG_R___) \
    do {NASM_EMIT("\tsub %s, %s\n", \
        REG_NAMES[(___REG_L___)], REG_NAMES[(___REG_R___)]);\
    emit_sub(compiler->code_section, ___REG_L___, ___REG_R___);} while (0)


#define EMIT_SUB_RSP_CONST(___CONST___) \
    do {NASM_EMIT("\tsub rsp, %ld\n", (___CONST___));\
    emit_sub_rsp_const(compiler->code_section, (___CONST___));} while (0)
    
#define EMIT_IMUL_REG_REG(___REG_L___, ___REG_R___) \
    do {NASM_EMIT("\timul %s, %s\n", \
        REG_NAMES[(___REG_L___)], REG_NAMES[(___REG_R___)]);\
    emit_imul(compiler->code_section, ___REG_L___, ___REG_R___);} while (0)

#define EMIT_IDIV_REG(___REG___)\
    do {NASM_EMIT("\tidiv %s\n", REG_NAMES[(___REG___)]);\
    emit_idiv(compiler->code_section, ___REG___);} while (0)
    
#define EMIT_CQO()\
    do {NASM_EMIT("\tcqo\n");\
    emit_cqo(compiler->code_section);} while(0)

// ------------------------------- syscall ------------------------------------

#define EMIT_SYSCALL()\
    do {NASM_EMIT("\tsyscall\n");\
    emit_syscall(compiler->code_section);} while(0)
...
#define EMIT_TEST(___REG_L___, ___REG_R___)\
    ...
    REG_NAMES[(___REG_L___)], REG_NAMES[(___REG_R___)]);\
    emit_test(compiler->code_section, (___REG_L___), (___REG_R___));\
    } while (0)
